...
const API_KEY = 'F98MNOBMHZNH8H3N';
...
const displayResults = (el, data) => {
  const { '01. symbol': symbol, '05. price': price, '07. latest trading day': date } = data['Global Quote'];

  const priceElem = el.querySelector('.price');
  priceElem.innerHTML = `$${Number(price).toFixed(2)}`;
  const symbolElem = el.querySelector('.symbol');
  symbolElem.innerHTML = symbol.toUpperCase();
  const dateElem = el.querySelector('.date');
  dateElem.innerHTML = `${date} ${date.includes(':') ? date : ''}`;
};
